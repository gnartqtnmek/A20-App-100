...
from app.schemas.assignment import GradeCreate, GradebookResponse, GradeRead
...
async def list_my_grades(
    ...
    grades = await assignment_service.list_grades_for_user(db, current_user.id)
...
async def get_course_gradebook(
    ...
    return await assignment_service.get_course_gradebook(db, course_id)
